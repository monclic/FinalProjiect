    ...
    <div class="short-detailed">
        <div class="short-detailed-main">
            <div class="short-context">
                <div class="shrot-wrapper">
                    <div class="short-avatar">
                        <div class="bili-dyn-avatar" :style="{ backgroundImage: 'url(' + state.items.avatar + ')' }">
                            <div></div>
                        </div>
                    </div>
                    <div class="short-context-body">
                        <div class="short-context-header">
                            <div class="bili-dyn-title">
                                <span>{{ state.items.username }}</span>
                            </div>
                        </div>
                        <div ref="editor" class="short-context-textarea">
                            <!-- {{ state.items.shortContent }} -->
                        </div>
                        <!-- 图片区域 -->
                        <div v-if="state.items.imagesIs" class="short-images">
                            <div v-for="(item, index) in state.items.imagesList " :key="index" class="short-images-wrapper">
                                <div class="short-images-item" :style="{ backgroundImage: 'url(' + item + ')' }"></div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="reply-space">
                <replys :to-context-id="id" :to-context-type="2"></replys>
            </div>
        </div>
        <div class="short-detailed-aside">
            <div class="aside-wrapper">
                <!-- 点赞 -->
                <div @click="setLike(2, state.items.id)" :class="{ 'like': true, 'liked': isLike }">
                    <div v-show="state.items.likeIs" class="is_like">
                    ...
                    <div v-show="!state.items.likeIs" class="not_like">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd"
                    ...
                    <div class="side-toolbar__action__text">{{ state.items.likes }}</div>
                </div>
                <div @click="get_short_detail" class="to-reply">
                    ...
                    <div class="side-toolbar__action__text">{{ state.items.replys }}</div>
                </div>
            </div>
        </div>
    </div>
...
const id = ref(router.params.sdid)
const editor = ref<HTMLDivElement | null>(null);
const get_short_detail = async () => {
    let param = { shortId: router.params.sdid }
    http.get("/shorts/Short", param)
        .then((data: any) => {
            state.items = data
            editor.value!.innerHTML=state.items.shortContent
            // console.log(state.items);
            
        })
}

const setLike = (type: number, contextId: NumberConstructor) => {
    let param = { type: type, contextId: contextId }
    http.post("/likes/setLike", param)
    state.items.likeIs = !state.items.likeIs
    if (state.items.likeIs) {
        state.items.likes = state.items.likes + 1
    } else {
        state.items.likes = state.items.likes - 1
    }
}
