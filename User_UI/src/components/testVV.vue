<template>
  <el-button style=" padding-top: 4.6rem;" text @click="dialogTableVisible = true">
    open a Table nested Dialog
  </el-button>

  <el-dialog class="dialog" v-model="dialogTableVisible" title="">
    <!-- 编辑区 -->
        ...
        <div class="bili-pics-uploader__content">
          ...
          <div class="bili-pics-uploader__item success" v-for="(item, index) in images" :key="index" style="">
            ...
            <div class="bili-pics-uploader-item-preview" status="SUCCESS" msg="">
              <div class="bili-pics-uploader-item-preview__pic" :style="{ backgroundImage: 'url(' + item.url + ')' }">
              </div>
            </div>
          </div>
          <!-- 图片上传窗口 -->
    ...
    <div class="meta">
      ...
      <div class="publishing__headquarters">
        <button @click="cc"
          style="width: 70px; height: 30px; background-color: aqua; user-select: none;"></button>
      </div>
    </div>
  </el-dialog>
</template>
