    ...
    <div class="login-container">
      <div class="login-wrapper">
        <div class="login-box">
          <div class="header">
            <div class="login">
              <div  @click="switchSpace(true)" :class="{'title':true,'actived':isLogin}">登录</div>
            </div>
            <div style="width: 20px;"></div>
            <div class="register">
              <div  @click="switchSpace(false)" :class="{'title':true,'actived':!isLogin}">注册</div>
            </div>
          ...
          <div class="login-body">
            ...
            <div v-show="isLogin" class="login-space">
              <form class="tab__form">
                ...
                <div class="form__item">
                  ...
                  <div @click="to_find_pwd" class="clickable">忘记密码？</div>
                </div>
              </form>
            ...
            <div v-show="!isLogin" class="register">
              <form class="tab__form">
                <div class="form__item">
                  <div>email</div>
                  <input v-model="email" maxlength="30" oninput="value=value.replace(/\s+/g, '')" @input="checkEmail" placeholder="请输入email">
                  <div class="login-sms-wp__vertical-line"></div>
                  <div @click="getCode" :class="{'login-sms-send':true, 'clickable':true, 'disable':!email_is||countdown>0}">
                ...
                <div class="form__item">
                  <div class="form_info">密码</div>
                  <input v-model="pwd" autocomplete="on" maxlength="32"
                    oninput="value=value.replace(/\s+/g, '')" placeholder="请输入密码">                                         
                </div>   
                <div class="form__separator-line"></div>
                <div class="form__item">
                  <div class="form_info">密码</div>
              ...
              <div   class="btn_wp" style="justify-content: center;"><!---->
                <div @click="register" class="btn_primary "> 注册 </div>
              </div>
            </div>
          </div>
        </div>
...
setTimeout(() => {
  ...
  const code_post=async()=>{
    ...
    http.post('/user/sendCode',email.value,{
    headers: {
  ...
  .title{
   text-decoration: none;color: #c9ccd0;
   cursor: pointer;
   user-select: none;
  }
  .actived{
