<template>
    <div class="container">
        <div class="publish">
            <el-input style="width: 900px;margin-left: 30px;"
    v-model="textarea"
    :rows="2"
    type="textarea"
    placeholder="Please input"
  />
  <div @click="publish" style="width: 90px;height: 50px;border-radius: 10px;margin-left: 10px; background-color: #72c7ea;color:#fff;text-align: center;padding-top: 10px;font-weight: 400;font-size: x-large;">发布</div>
        </div>
  ...
  const delete_do = () => {
    let params = { id: sid.value }
    
    http.post('/board/delete', sid.value,{
      headers: {
  ...
  const textarea = ref('')
const publish=()=>{
    http.post('/board/add',textarea.value,{
    headers: {
      'Content-Type': 'text/plain' // 指定请求体的内容类型为纯文本
    }
  })
